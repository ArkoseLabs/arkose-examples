 ...
 * Cloudflare CDN layer with Data Exchange.
 *
...
const encrypt = async (keyBytes, data) => {
  ...
  const iv = crypto.getRandomValues(buffer);
...
const retrieveUsername = async (response) => {
  ...
  const position = originalBody.search(usernameBodyText);

  if (position < 0) {
    return '';
  }

  const endPosition = originalBody.indexOf(
    '"',
    position + usernameBodyText.length
  );
  const username = originalBody.substring(
    position + usernameBodyText.length,
    endPosition
  );
...
export default {
  async fetch(request, env) {
    ...
    const usingDX = parseBoolean(env.usingDX);
    const tokenEnforcement = parseBoolean(env.tokenEnforcement);
    const secretKey = new Uint8Array(
      Buffer.from(env.secretKeyBase64, 'base64')
    );
    ...
    const injectArkose = async (response, username) => {
      const blob = usingDX
        ? await dataExchangeHandler(secretKey, username)
        : '';
